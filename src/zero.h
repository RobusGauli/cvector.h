...
#define zero__free(zero)                                                       \
  do {                                                                         \
    ...
    for (int i = 0; i < block_size; i++) {                                     \
      typeof(((zero)->next_holder)) mem =                                      \
          vector__index(&((zero)->vector_type_pointer), i);                    \
      vector__free(*(mem));                                                    \
    }                                                                          \
...
#define zero_iterator__done(it)                                                \
  ((((it)->ty) != NULL) ? ((it)->current_index >= zero__size((it)->ty)) : true)

#define zero_iterator__current_index(it) ((it)->current_index)

#define zero_iterator__next(it)                                                \
  ((((it)->current_index)++),                                                  \
   (zero__index(((it)->ty), ((it)->current_index) - 1)))

#define zero_iterator__next_cpy(it)                                            \
  ((((it)->current_index)++),                                                  \
   (zero__index_cpy(((it)->ty), ((it)->current_index) - 1)))

#define zero_iterator__peek(it) (zero__index(((it)->ty), ((it)->current_index)))

#define zero_iterator__peek_cpy(it)                                            \
  (zero__index_cpy(((it)->ty), ((it)->current_index)))
